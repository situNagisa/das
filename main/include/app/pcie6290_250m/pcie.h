...
struct instance
{
	instance()
	{
		resize(_configurator.info().packet_size, _saver.info().process_frame);
	...
	{
		return collect_times * _configurator.info().scan_rate / _saver.info().process_frame;
	}
	decltype(auto) _frame_to_times(::std::size_t hold_frame) const
	{
		return (hold_frame * _saver.info().process_frame / _configurator.info().scan_rate);
	}
	void update_saver()
	{
