...
struct buffer
{
	...
	auto flag()const { return ::edfa::protocols::flag(static_cast<::edfa::protocols::flag::type>(::std::to_integer<::std::uint16_t>(_data[0]) | (::std::to_integer<::std::uint16_t>(_data[1]) << 8))); }
	auto length()const { return ::edfa::protocols::length(::std::to_integer<::std::uint8_t>(_data[2])); }
	::std::size_t data_length() const { return length().value() - 3; }
	auto data_current_length() const { return size_data().size() - 5; }
	auto address()const { return ::edfa::protocols::address(static_cast<::edfa::protocols::address::type>(::std::to_integer<::std::uint8_t>(_data[3]))); }
	auto response()const { return ::edfa::protocols::response(static_cast<::edfa::protocols::response::type>(::std::to_integer<::std::uint8_t>(_data[4]))); }
...
inline struct _buffer_checker_function
{
	...
	_buffer_checker_function()
	{
		//flag
		_handlers.push_back({
			.check = [](const buffer& buffer, ::std::byte new_byte) { return new_byte == static_cast<::std::byte>(static_cast<::std::uint16_t>(::edfa::protocols::flag::type::receive) >> (8 * buffer.size_data().size())); },
