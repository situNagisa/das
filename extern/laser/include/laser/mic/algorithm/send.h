...
auto send(protocols::address address, protocols::command command, ::std::ranges::input_range auto&& data, ::std::output_iterator<::std::uint8_t> auto&& result)
	requires ::std::convertible_to<::std::ranges::range_value_t<decltype(data)>, uint8_t>&& ::std::indirectly_copyable<::std::ranges::range_value_t<decltype(data)>,decltype(result)>
{
	...
	auto checksum = protocols::checksum(protocols::flag::send, length, address, command);
