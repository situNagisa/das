...
namespace c_api
{
	...
	{
		_config.channel_quantity = static_cast<enums::channel_quantity>(demodulation_ch_quantity);
		return 0;
	}

	NGS_HPP_INLINE int pcie6920_set_points_num_per_scan(unsigned point_num_per_scan)
	{
		_config.packet_size = point_num_per_scan;
		return 0;
	}
	...
	{
		_config.pulse_width = pulse_high_width_ns;
		return 0;
	}
	...
	{
		_config.parse_rule = static_cast<enums::parse_rule>(data_src_sel);
		return 0;
	}

	NGS_HPP_INLINE int pcie6920_upload_rate_sel(unsigned upload_rate_sel)
	{
		_config.upload_rate = static_cast<enums::upload_rate>(upload_rate_sel);
		return 0;
	}

	NGS_HPP_INLINE int pcie6920_point_num_per_ch_in_buf_query(unsigned* p_point_num_in_buf_per_ch)
	{
		*p_point_num_in_buf_per_ch = 0x100 * 1000000;
		return 0;
	}
	...
	NGS_HPP_INLINE int pcie6920_read_data(unsigned point_num_per_ch, short* p_raw_data, unsigned* p_points_per_ch_returned)
	{
		...
		if (_config.parse_rule == enums::parse_rule::raw_data)
		{
